...
if __name__ == "__main__":
    ...
    print(
        "PELT",
        algo_changepoints.pelt(pd.DataFrame(TIME_SERIES), PEN_, MINSEG, METHOD),
        time.time() - START,
    )
    START = time.time()
    print(
        "BinSeg",
        algo_changepoints.binseg(pd.DataFrame(TIME_SERIES), 10, MINSEG, METHOD),
        time.time() - START,
    )
    START = time.time()
    print(
        "NPPELT",
        algo_changepoints.np_pelt(
            pd.DataFrame(TIME_SERIES), PEN_ * np.log(SIZE_TS), 10
        ),
        time.time() - START,
    )
    START = time.time()
    print(
        "Segneigh",
        algo_changepoints.segneigh(pd.DataFrame(TIME_SERIES), 10, METHOD),
        time.time() - START,
    )
